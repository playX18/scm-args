...
(define (handle-long-option parser name value)
  ...
  (cond 
    ...
    ((string-prefix? "no-" name)
      (let* ((positive-value (substring name 3))
  ...
  (cond 
    ...
    (else 
      (let* ((index (string-index (current parser) #\=))
             (name (if index 
                      (substring (current parser) 2 index)
                      (substring (current parser) 2))))
        (cond 
          ...
          (else 
            (let ((value (if index 
                           (substring (current parser) (+ 1 index))
                           #f)))
...
(define (handle-abbreviation parser letters-and-digits rest innermost-command)
  ...
  (cond 
    ...
    ((not (option-flag? (cdr first)))
      (let ((value (string-append (substring letters-and-digits 1) rest)))
...
(define (parse-solo-option parser)
  (cond 
    ...
    (else 
      (let ((opt (substring (current parser) 1)))
